...
INTERNAL OpenGLInfo
opengl_get_info(void)
{
  OpenGLInfo result = ZERO_STRUCT;
...
INTERNAL GLuint
opengl_create_program(char *header_code, char *fragment_code)
{
  GLuint result = 0;

  ASSERT(header_code != NULL);
  ASSERT(fragment_code != NULL);

  GLuint fragment_shader_id = glCreateShader(GL_FRAGMENT_SHADER);
  GLchar *fragment_shader_code[2] = {header_code, fragment_code};
  glShaderSource(fragment_shader_id, 2, fragment_shader_code, NULL);
  ...
  GLint linked_successfully = false;
  ...
  if (!linked_successfully)
  {
    ...
    char program_errors[4096] = ZERO_STRUCT;
...
opengl_init(void)
{
  ...
  result.program_id = opengl_create_program(header_code, fragment_code);
  result.time_id = glGetUniformLocation(result.program_id, "time");
  result.resolution_id = glGetUniformLocation(result.program_id, "resolution");
